  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      ...
      if margin > 0:
        ...
        #######################################################################
        # only non-zero loss contributes to gradient
        # for class j, there will be gradient contribution to
        # the j^th and the y[i]^th column
        dW[:,j] += X[i].T / num_train # add X[i]
        dW[:,y[i]] -= X[i].T / num_train # minus X[i]
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # result in loss.                                                           #
  #############################################################################
  score_matrix = X.mm(W) # dimension N x C
  # for the i^th data point, the correct label is y[i] and 
  # the score is score_matrix[i,y[i]]
  row_idx = [i for i in range(num_train)]
  true_class_idx = [y[i] for i in range(num_train)]
  # convert into column vector for broadcasting
  score_correct_label = score_matrix[row_idx, true_class_idx].reshape(-1,1) 
  raw_loss = score_matrix - score_correct_label + 1
  raw_loss[raw_loss < 0] = 0
  # subtract 1 because need to remove the loss contribution from the case
  # where class j = class y[i]
  loss = raw_loss.sum() / num_train - 1
  ...
  #############################################################################
  # dimension N x C
  non_zero = torch.zeros_like(score_matrix)
  non_zero[raw_loss > 0] = 1
  
  # count the number of non-zero scores for each training instance
  num_nonzero = non_zero.sum(axis=1)
  scaled_true_class = torch.zeros_like(score_matrix) # dimension N x C
  scaled_true_class[row_idx, true_class_idx] = num_nonzero

  # no need to worry about the extra (s_yj - s_yj + 1) term
  # its net gradient contribution will be 0
  dW = X.T.mm(non_zero - scaled_true_class) / num_train
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  rand_idx = torch.randint(0, num_train, (batch_size,))
  X_batch = X[rand_idx, :]
  y_batch = y[rand_idx]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  prediction_scores = X.mm(W) # dimension N x C
  y_pred = torch.argmax(prediction_scores, axis=1)
...
def svm_get_search_params():
  ...
  regularization_strengths = [1, 10, 100, 1000]
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  _ = cls.train(data_dict['X_train'],data_dict['y_train'], 
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    for j in range(num_class):
      scores = W.T.mv(X[i]) # X[i]: (D,)
      normalization_factor = -scores.max() # log(C) from the link
      exp_scores = (scores + normalization_factor).exp()
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = X.mm(W) # dimension (NxC)
  # find row wise max
  normalization_factors = (-scores.max(axis=1).values).reshape(-1,1) 
  ...
  indicator_matrix[row_idx, correct_class_idx] = 1
  dW = X.T.mm(probs - indicator_matrix) / num_train
