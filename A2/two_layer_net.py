...
def nn_forward_pass(params, X):
    ...
    # shape (N, C).                                                            #
    ############################################################################
    # clamp(0) is equivalent ReLU
    # X.mm(W1) is (NxH) and b1 is (H,) => broadcasting works
    hidden = (X.mm(W1) + b1).clamp(0) 
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # softmax loss with L2
    L2_reg = W2.square().sum() + W1.square().sum()
    
    normalization_factor = -scores.max(axis=1, keepdim=True).values
    exp_scores = (scores + normalization_factor).exp()
    prob = exp_scores / exp_scores.sum(axis=1, keepdim=True)
    log_loss = -prob[torch.arange(N), y].log().sum() / N
    loss = log_loss + reg * L2_reg
    ...
    # tensor of same size                                                     #
    ###########################################################################
    grad_scores = prob.clone()
    grad_scores[torch.arange(N), y] -= 1 
    grad_scores /= N
    
    grads["b2"] = grad_scores.sum(axis=0)
    grads["W2"] = h1.T.mm(grad_scores) + 2 * reg * W2
    grad_h = grad_scores.mm(W2.T)
    # this is the gradient of the pre-ReLU transformed hidden layer
    grad_preh = grad_h.clone()
    grad_preh[h1==0] = 0
    grads["W1"] = X.T.mm(grad_preh) + 2 * reg * W1
    grads["b1"] = grad_preh.sum(axis=0)
