...
def compute_saliency_maps(X, y, model):
  ...
  raw_scores = model(X) # un-normalized scores
  correct_scores = raw_scores[:,y]
  correct_scores.sum().backward()
  saliency, _  = X.grad.max(axis=1)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  while True:
    loop_idx += 1
    raw_scores = model(X_adv)
    raw_scores[0, target_y].backward()
    
    if raw_scores.argmax().item() == target_y:
      break
    
    target_score = raw_scores[0, target_y]
    max_score = raw_scores.max().item()
    print(f'Iteration {loop_idx}: target score: {target_score:.2f}, max score {max_score:.2f}')

    with torch.no_grad():
      grad = X_adv.grad
      X_adv += learning_rate * grad / grad.square().sum().sqrt()
      X_adv.grad.zero_()
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    raw_scores = model(img)
    correct_score = raw_scores[0, target_y] - l2_reg * img.square().sum()
