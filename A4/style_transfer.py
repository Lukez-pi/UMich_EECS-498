...
def content_loss(content_weight, content_current, content_target):
    ...
    ##############################################################################
    content_loss = None
    content_loss = (content_current - content_target).square().sum()
...
def gram_matrix(features, normalize=True):
    ...
    gram = features_flat.bmm(features_flat.transpose(1,2))
    if normalize:
      gram /= (H * W * C)
    return gram
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer in enumerate(style_layers):
      style_current = gram_matrix(feats[layer].clone())
      style_loss += style_weights[idx] * (style_targets[idx] - style_current).square().sum() 
...
def tv_loss(img, tv_weight):
    ...
    tv_loss = 0
    _, _, H, W = img.shape
    tv_loss += (img[:, :, 1:, :] - img[:, :, :-1, :]).square().sum() 
    tv_loss += (img[:, :, :, 1:] - img[:, :, :, :-1]).square().sum()
    tv_loss *= tv_weight
    return tv_loss