...
def three_layer_convnet(x, params):
  ...
  conv2_output = flatten(conv2_output)
  scores = F.linear(conv2_output, fc_w, fc_b)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  pad_1, pad_2 = 2, 1
  H1_out, W1_out = (H + 2 * pad_1 - kernel_size_1) + 1, (W + 2 * pad_1 - kernel_size_1) + 1
  H2_out, W2_out = (H1_out + 2 * pad_2 - kernel_size_2) + 1, (W1_out + 2 * pad_2 - kernel_size_2) + 1
  fc_dim = channel_2 * H2_out * W2_out

  conv_w1 = nn.init.kaiming_normal_(
    torch.empty(channel_1, C, kernel_size_1, kernel_size_1, dtype=dtype, device="cuda")
  )
  conv_w1.requires_grad = True

  conv_b1 = nn.init.zeros_(
    torch.empty(channel_1, dtype=dtype, device="cuda")
  )
  conv_b1.requires_grad = True

  conv_w2 = nn.init.kaiming_normal_(
    torch.empty(channel_2, channel_1, kernel_size_2, kernel_size_2, dtype=dtype, device="cuda")
  )
  conv_w2.requires_grad = True

  conv_b2 = nn.init.zeros_(
    torch.empty(channel_2, dtype=dtype, device="cuda")
  )
  conv_b2.requires_grad = True

  fc_w = nn.init.kaiming_normal_(
    torch.empty(num_classes, fc_dim, dtype=dtype, device="cuda")
  )
  fc_w.requires_grad = True
  
  fc_b = nn.init.zeros_(
    torch.empty(num_classes, dtype=dtype, device="cuda")
  )
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    kernel_1, kernel_2 = 5, 3
    pad_1, pad_2 = 2, 1
    H, W = 32, 32 # assumption of the model
    H1_out, W1_out = (H + 2 * pad_1 - kernel_1) + 1, (W + 2 * pad_1 - kernel_1) + 1
    H2_out, W2_out = (H1_out + 2 * pad_2 - kernel_2) + 1, (W1_out + 2 * pad_2 - kernel_2) + 1
    fc_dim = channel_2 * H2_out * W2_out

    self.conv1 = nn.Conv2d(in_channel, channel_1, kernel_1, padding=pad_1)
    self.conv2 = nn.Conv2d(channel_1, channel_2, kernel_2, padding=pad_2)
    self.fc = nn.Linear(fc_dim, num_classes)

    # kaiming initialization on weights
...
def initialize_three_layer_conv_part4():
  ...
  ####################################################################################
  H1_out, W1_out = (H + 2 * pad_size_1 - kernel_size_1) + 1, (W + 2 * pad_size_1 - kernel_size_1) + 1
  H2_out, W2_out = (H1_out + 2 * pad_size_2 - kernel_size_2) + 1, (W1_out + 2 * pad_size_2 - kernel_size_2) + 1
  fc_dim = channel_2 * H2_out * W2_out
  ...
  model = nn.Sequential(OrderedDict([
    ...
    ('relu2', nn.ReLU()),
    ('flatten', Flatten()),
    ('fc', nn.Linear(fc_dim, num_classes)),
  ]))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(OrderedDict([
      ...
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout, 3, stride=self.stride, padding=1)),
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    self.stage = [ResNetStage(*arg, block) for arg in stage_args]
    self.cnn = nn.Sequential(self.stem, *self.stage)
  ...
  def forward(self, x):
    ...
    scores = self.fc(flatten(torch.mean(self.cnn(x), axis=(2,3))))
    return scores
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(OrderedDict([
      ('batchnorm1', nn.BatchNorm2d(Cin)), 
      ('ReLU1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout//4, 1, stride=stride)), 
      ('batchnorm2', nn.BatchNorm2d(Cout//4)),
      ('ReLU2', nn.ReLU()),
      ('conv2', nn.Conv2d(Cout//4, Cout//4, 3, padding=1)),
      ('batchnorm3', nn.BatchNorm2d(Cout//4)),
      ('ReLU3', nn.ReLU()),
      ('Conv3', nn.Conv2d(Cout//4, Cout, 1))
    ]))
