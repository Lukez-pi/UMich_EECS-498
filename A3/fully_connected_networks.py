...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    out_l2, cache_l2 = Linear.forward(out_l1, self.params["W2"], self.params["b2"])
    # out_l2 is of dimension (NxC)
    # no need to calculate softmax ourselves, it is already implemented in the `softmax_loss` function
    scores = out_l2
    ...
    W2 = self.params["W2"] # cache_l2[1]
    W1 = self.params["W1"] # cache_l1[0][1]
    loss += self.reg * (W2.square().sum() + W1.square().sum())

    dh, dw2, db2 = Linear.backward(dscore, cache_l2)
    _, dw1, db1 = Linear_ReLU.backward(dh, cache_l1)
    grads["W2"] = dw2 + 2 * self.reg * W2
    grads["b2"] = db2 
    # cache_l1 = (fc_cache, relu_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for layer_num, hd in enumerate(dims, 1):
      self.params[f"b{layer_num}"] = torch.zeros(hd, dtype=dtype, device=device)
      dim1 = input_dim if layer_num == 1 else dims[layer_num-2]
      self.params[f"W{layer_num}"] = weight_scale * torch.randn(dim1, hd, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    prev_out = X
    # a better approach probably is to use a dictionary to hold the caches
    caches = []
    for i in range(self.num_layers):
      ...
      if layer < self.num_layers:
        out, cache = Linear_ReLU.forward(prev_out, W, b)
        if self.use_dropout:
          out, cache_dropout = Dropout.forward(out, self.dropout_param)
          caches.append([cache, cache_dropout])
        else:
          caches.append(cache)
        # dropout_param, cache
      else: # last layer, there's no dropout with the last layer
        out, cache = Linear.forward(prev_out, W, b)
        caches.append(cache)
      prev_out = out
    ...
    upstream_gradient = dscore
    for i in range(self.num_layers):
      layer = self.num_layers - i
      c = caches[layer-1]
      W = self.params[f"W{layer}"]
      if layer == self.num_layers: # last layer, linear
        dh, dw, db = Linear.backward(upstream_gradient, c)  
      else:
        if self.use_dropout:
          c, c_dropout = c[0], c[1]
          upstream_gradient = Dropout.backward(upstream_gradient, c_dropout)
        dh, dw, db = Linear_ReLU.backward(upstream_gradient, c)
      upstream_gradient = dh
      grads[f"W{layer}"] = dw + self.reg * W
      grads[f"b{layer}"] = db
      loss += 0.5 * self.reg * (W.square().sum())
...
def rmsprop(w, dw, config=None):
  ...
  ###########################################################################
  cache = config["cache"]
  cache = config["decay_rate"] * cache + (1 - config["decay_rate"]) * dw.square()
...
def adam(w, dw, config=None):
  ...
  epsilon, learning_rate = config["epsilon"], config["learning_rate"]
  m, v = config["m"], config["v"]
  M1 = beta1 * m + (1 - beta1) * dw
  M2 = beta2 * v + (1 - beta2) * dw.square()
  M1_unbiased = M1 / (1 - beta1**t)
  M2_unbiased = M2 / (1 - beta2**t)
  next_w = w - learning_rate * M1_unbiased / (M2_unbiased.sqrt() + epsilon)

  config["m"], config["v"] = M1, M2
  return next_w, config
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      mask = (mask > p) # only keep the ones > p, expected value is (1-p)
      out = x * mask / (1-p)
