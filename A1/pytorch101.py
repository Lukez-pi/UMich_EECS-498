...
def create_sample_tensor():
  ...
  #############################################################################
  x = torch.tensor([[0,0] for i in range(3)])
  x[0,1] = 10
  x[1,0] = 10
...
def multiples_of_ten(start, stop):
  ...
  if stop - start < 10:
    ...
    x = torch.arange(10*(1+start//10), stop+1, 10).to(torch.float64)
  return x
...
def reverse_rows(x):
  ...
  #############################################################################
  row_num = x.shape[0]
  y = x[[i for i in range(row_num-1,-1,-1)]]
  return y
...
def take_one_elem_per_col(x):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  y = x[[1,0,3], [i for i in range(x.shape[1])]]
...
def make_one_hot(x):
  ...
  #############################################################################
  C = 1 + max(x)
  y = torch.zeros(len(x), C, dtype=torch.uint8)
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  B, N, P = x.shape[0], x.shape[1], y.shape[-1]
  if use_loop:
    z = torch.empty(B,N,P)
    for i in range(B):
      z[i] = x[i].mm(y[i])
...
def normalize_columns(x):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  # x has dimension (num_row, num_col)
  y = x.clone()
  mean = torch.mean(x, axis=0) # rank 1, dimension of (num_col,)
