...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      train_instance = x_train[i,:].view(-1)
      test_instance = x_test[j,:].view(-1)
      d = (train_instance - test_instance).pow(2).sum(axis=0).sqrt()
      dists[i,j] = d.item()
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  for i in range(num_train):
    # single train instance, 3D matrix (3x32x32)
    train_instance = x_train[i,:]
    # the test matrix is of dimension (num_testx3x32x32), compatible for broadcasting
    dists[i,:] = (x_test - train_instance).pow(2).sum(axis=[1,2,3]).sqrt()
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # the original approach: but was too memory inefficient
  # x_train.view(num_train,1,-1) - x_test.view(1,num_test,-1)).pow(2).sum(axis=2).sqrt()
  # instead expand the sum of differences as: (a-b)^2 = a^2 - 2ab + b^2
  A = x_train.reshape(num_train, -1)
  B = x_test.reshape(num_test, -1)
  A2 = A.pow(2).sum(axis=1).view(num_train,1) # dimension (num_train, 1)
  B2 = B.pow(2).sum(axis=1).view(1,num_test) # dimension (1, num_test)
  AB = A.mm(B.T) # dimension num_train x num_test
  dists = (A2 + B2 - 2*AB).sqrt()
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # dists: (num_train x num_test)
  topk_indices = torch.topk(dists, k, dim=0, largest=False).indices
  y_pred[:] = y_train[topk_indices].mode(axis=0).values
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # Hint: torch.chunk                                                          #
  ##############################################################################
  # torch.chunk does not shuffle, it just chunks elements in sequential order
  ...
  for k in k_choices:
    for test_fold_idx in range(num_folds):
      print(f"k: {k}, fold_idx: {test_fold_idx}", end=", ")
      train_fold_indices = [i for i in range(num_folds) if i != test_fold_idx]
      x_train_f = torch.cat([x_train_folds[i] for i in range(num_folds) if i !=test_fold_idx])
      y_train_f = torch.cat([y_train_folds[i] for i in range(num_folds) if i !=test_fold_idx])
      
      x_test_f = x_train_folds[test_fold_idx]
      y_test_f = y_train_folds[test_fold_idx]

      classifier = KnnClassifier(x_train_f, y_train_f)
      accuracy = classifier.check_accuracy(x_test_f, y_test_f, k=k, quiet=False)
...
def knn_get_best_k(k_to_accuracies):
  ...
  best_k = ks[torch.argmax(torch.tensor(means))]
